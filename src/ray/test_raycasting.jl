...
# include("simulation/util_octtree.jl")
...
mesh = load("/Users/jakoo/Desktop/blender_files/bunny.obj")

vs_ = mesh.vertices
min_ = minimum(vs_)
max_ = maximum(vs_)
vs = Base.map(x -> (x - min_) ./ (max_ - min_) .* 2 .- 1 , vs_)
...
sinogram, tt = fp_mesh(proj_geom, vs, mesh.faces)
